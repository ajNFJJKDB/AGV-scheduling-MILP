...
initial_charge = {row['AGV_ID']: row['Initial charge'] for index, row in agv_properties_df.iterrows()}  # Ensure minimum 70% charge
...
discharging_rate = 0.1  # 0.1% per second
...
# Objective: Minimize the makespan and balance the load among AGVs, and penalize assigning normal tasks to heavy AGVs
penalty = 1000  # Define a penalty factor for assigning normal tasks to heavy AGVs
objective = gp.quicksum(end_time[i] for i in range(n_tasks)) + gp.quicksum(agv_load[j] for j in range(n_agvs))
objective += gp.quicksum(penalty * x[i, j] for i in range(n_tasks) for j in range(n_agvs) if service_requests_df.loc[i, 'Capability Requirement'] == 'Normal' and agv_capabilities[j] == 'Heavy')
...
for j in range(n_agvs):
    ...
    for i in range(n_tasks):
        ...
        model.addConstr(charge_level[i + 1, j] >= 1, f"Non_negative_charge_{i}_{j}")
    ...
    for j in range(n_agvs):
        ...
        for i in range(n_tasks):
            if solution[i, j] > 0.5:
                ...
                # Charge if there is idle time and AGV doesn't have sufficient charge
                if idle_time > 0 and current_charge_level[j] < required_charge:
                ...
                # Ensure AGV starts the task immediately if it has enough initial charge
                if previous_end_time == 0 and current_charge_level[j] >= required_charge:
                    start = 0
